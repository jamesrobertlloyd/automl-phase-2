...
# ========= Convenience functions written by James Robert Lloyd April 2015 =========
...
def eval_metric(metric, truth, predictions, task):
    """General purpose metric evaluation function - avoiding using dirty eval statements"""
    metric = metric.lower()
    if metric == 'sklearn auc':
        truth, predictions = ensure_2d(truth), ensure_2d(predictions)
        truth = truth[:, -1]
        predictions = predictions[:, -1]
        # try:
        return metrics.roc_auc_score(truth, predictions)
        # except:
        #     return np.nan
    elif metric == 'automl auc':
        # [truth, predictions] = normalize_array(ensure_2d(truth), ensure_2d(predictions))
        return auc_metric(ensure_2d(truth), ensure_2d(predictions), task)
    elif metric == 'automl bac':
        return bac_metric(ensure_2d(truth), ensure_2d(predictions), task)
    elif metric == 'automl pac':
        return pac_metric(ensure_2d(truth), ensure_2d(predictions), task)
    elif metric == 'automl r2':
        truth = sanitize_array(truth)
        predictions = sanitize_array(predictions)
        return r2_metric(truth, predictions, task)
    elif metric == 'automl f1':
        return f1_metric(ensure_2d(truth), ensure_2d(predictions), task)
    elif metric == 'automl a':
        truth = sanitize_array(truth)
        predictions = sanitize_array(predictions)
        return a_metric(truth, predictions, task)
    else:
        raise Exception('I do not know how to evaluate metric : %s' % metric)
...
def show_io(input_dir, output_dir):  
	...
	# List input and predictions directories
